  ...
  | Lhandle (exp1, tst) =>
    let
      fun compileTsts tst tsts = 
        case tst of
          (Lif(Lprim(Ptest(Peq_test), [arg1 as (Lprim(_,[Lvar(j)])), arg2]), exp2, exp3)) =>
            compileTsts exp3 ((JSGetVar(nth(env',j)), compileJSLambda arg1 env', 
                 compileJSLambda arg2 env', compileJSLambda exp2 env')::tsts)
        | _ => tsts
